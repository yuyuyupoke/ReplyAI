...
async function rateComment(commentId, action, btnElement) {
    try {
        // Find parent container to get current state
        const metaDiv = btnElement.closest('.comment-meta-right');
        const currentRating = metaDiv.getAttribute('data-rating') || 'none';
        let newRating = action;

        // Toggle logic with mutual exclusivity
        if (currentRating === action) {
            newRating = 'none'; // Toggle off
        } else {
            // If switching from like to dislike or vice versa, newRating is already set to action
            // The server handles the switch, but we need to update UI carefully
        }

        console.log(`Toggling rating: ${currentRating} -> ${newRating}`);

        const response = await fetch('/rate_comment', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ comment_id: commentId, rating: newRating }),
        });

        const data = await response.json();
        if (data.status === 'success') {
            // Update UI state
            metaDiv.setAttribute('data-rating', newRating);

            // Update buttons active state
            const buttons = metaDiv.querySelectorAll('.btn-icon');
            buttons.forEach(btn => btn.classList.remove('active'));

            if (newRating === 'like') {
                // Assuming first button is like
                buttons[0].classList.add('active');
            } else if (newRating === 'dislike') {
                // Assuming second button is dislike
                buttons[1].classList.add('active');
            }

            // Update like count
            const likeCountSpan = metaDiv.querySelector('.like-count');
            if (likeCountSpan) {
                let currentText = likeCountSpan.textContent;
                let match = currentText.match(/(\d+)/);
                let currentCount = match ? parseInt(match[0]) : 0;

                // Calculate delta
                let delta = 0;
                if (currentRating === 'like') delta--;
                if (newRating === 'like') delta++;

                currentCount += delta;

                likeCountSpan.textContent = `いいね数 ${currentCount}`;

                // Visual feedback
                likeCountSpan.style.color = 'red';
                setTimeout(() => { likeCountSpan.style.color = ''; }, 1000);
            }
        } else {
            alert('操作に失敗しました: ' + (data.message || '不明なエラー'));
        }
    } catch (error) {
        console.error(error);
        alert('エラーが発生しました: ' + error);
